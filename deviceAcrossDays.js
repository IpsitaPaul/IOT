...
$('.get-data').click(function() {
  ...
  var forminputDate = $('.select-start select').find(":selected").text();
  var metric = $('.select-end select').find(":selected").text();

  //check user inputs
  if (formDeviceId == "") {
    alert("Select a device");
    return;
  } else if (forminputDate.includes('[choose date]')) {
    alert("Select start date");
    return;
  } else if (metric.includes('[choose date]')) {
    alert("Select metric");
    return;
  } 

  //create json data
  var inputData = '{' + '"deviceId" : "' + formDeviceId + '", ' + '"inputDate" : "' + forminputDate + '", ' + '"metric" : "' + metric + '"}';
...
function plotAcrossDays(data) {
  ...
  var layout = {
    title: "Device " + id + " from " + inputDate + " to " + metric
  };
